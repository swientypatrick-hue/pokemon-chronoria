...
import { resolveText, type TranslationContext, type TranslatedText } from "./translationContext.ts";
...
function blockToMove(block: PbsBlock, description: TranslatedText): Move {
...
export function parseMoves(ctx: TranslationContext): Move[] {
  ...
  for (const block of parsePbsBlocks(load("moves.txt"))) {
    const r = blockToRecord(block);
    moves.set(block.headerParts[0], blockToMove(block, resolveText(ctx.moveDescriptionText, r.Description)));
  }
  ...
  for (const block of parsePbsBlocks(load("moves_Gen_9_Pack.txt"))) {
    const r = blockToRecord(block);
    moves.set(block.headerParts[0], blockToMove(block, { text: r.Description ?? "", fallback: false }));
  }
